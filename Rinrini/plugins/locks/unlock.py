...
from Rinrini.database.locks_mongo import unlock_db
from Rinrini.helper import custom_filter
from Rinrini.helper.chat_status import check_bot, check_user

from . import lock_map


@RinriniCli.on_message(custom_filter.command(commands=('unlock')))
async def lock(client, message):
    ...
    if 'all' in LOCK_ITMES:
        try:
            await RinriniCli.set_chat_permissions(
                chat_id,
                ChatPermissions(
