...
from Rinrini.database.locks_mongo import get_allowlist, get_locks, lockwarns_db
from Rinrini.helper.chat_status import check_bot, isUserAdmin
from Rinrini.plugins.warnings.warn import warn
from urlextract import URLExtract

from . import lock_map


@RinriniCli.on_message(filters.all & (filters.group | filters.channel) | filters.new_chat_members, group=4)
    ...
    if 7 in LOCKS_LIST:
        if (
            ...
            if message.reply_to_message.forward_from_chat.type == 'channel':
                ...
                chat_data = await RinriniCli.get_chat(
                    chat_id=chat_id
                )
                linked_chat = chat_data.linked_chat.id
                if linked_chat == channel_id:
                    chat_member = await RinriniCli.get_chat_member(
                        chat_id=chat_id,
                        user_id=from_user
                    )
