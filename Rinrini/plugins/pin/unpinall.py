...
from Rinrini.helper.chat_status import CheckAllAdminsStuffs, isUserCan


@RinriniCli.on_message(custom_filter.command(commands=('unpinall')))
...
@RinriniCli.on_callback_query(filters.create(lambda _, __, query: 'unpin_' in query.data))
async def unpinall_callback(client: RinriniCli, callback_query: CallbackQuery):
    ...
    if not await isUserCan(callback_query, user_id=user_id, chat_id=chat_id, permissions='can_pin_messages', silent=True):
        ...
        return 
    if query_data == 'yes':
        await RinriniCli.unpin_all_chat_messages(
            chat_id=chat_id
        )
