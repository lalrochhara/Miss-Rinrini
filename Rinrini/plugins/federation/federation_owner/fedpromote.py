...
from Rinrini.database.federation_mongo import (fed_promote,
...
@RinriniCli.on_message(custom_filter.command(commands=('fedpromote')))
async def FedPromote(client, message):
    ...
    user = await RinriniCli.get_users(
        user_ids=user_id
    )
...
@RinriniCli.on_callback_query(filters.create(lambda _, __, query: 'fedpromote_' in query.data))
async def fedpromote_callback(client: RinriniCli, callback_query: CallbackQuery):
    ...
    owner = await RinriniCli.get_users(
        user_ids=owner_id
    )
