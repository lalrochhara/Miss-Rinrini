...
from Rinrini.database.welcome_mongo import (AppendVerifiedUsers,
...
async def textCaptchaRedirect(message):
    ...
    if _match in [
        ...
        if new_user_id == user_id:
            ...
            CaptchaLoc = f"Rinrini/plugins/greeting/captcha/CaptchaDump/RinriniCaptcha_text_{new_user_id}_{new_chat_id}.png"
...
@RinriniCli.on_callback_query(filters.create(lambda _,__, query: 'textc_' in query.data))
async def textCaptchaCallBack(client: RinriniCli, callback_query: CallbackQuery):
    ...
    if (
        ...
        await RinriniCli.edit_message_caption(
            chat_id=user_id,
