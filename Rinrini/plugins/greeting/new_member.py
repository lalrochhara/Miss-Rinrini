...
from Rinrini import BOT_ID, LOG_CHANNEL, OWNER_ID, SUDO_USERS, RinriniCli
from Rinrini.database.federation_mongo import (get_fed_from_chat,
                                              get_fed_reason, is_user_fban)
from Rinrini.database.welcome_mongo import (DEFAUT_WELCOME, GetCaptchaSettings,
...
from Rinrini.helper.chat_status import isBotCan, isUserAdmin
from Rinrini.helper.welcome_helper.welcome_fillings import Welcomefillings
from Rinrini.helper.welcome_helper.welcome_send_message import \
    SendWelcomeMessage
from Rinrini.plugins.greeting.captcha import button_captcha, text_captcha


@RinriniCli.on_message(filters.new_chat_members)
async def NewMemeber(client, message):
    ...
    for NewUserJson in message.new_chat_members:
        ...
        # Rinrini's Special welcome for kami-samas!
        ...
        # Rinrini's Special welcome for her onii-chan gang!
        ...
        if is_user_fban(fed_id, user_id):
                ...
                if await isBotCan(message, permissions='can_restrict_members'):
                    if await RinriniCli.kick_chat_member(chat_id, user_id): 
                        text += '\nAction: `Banned`'
        ...
        if isWelcome(chat_id):
            ...
            WelcomeSentMessage = await RinriniCli.send_message(
                chat_id=chat_id,
                text=Text,
