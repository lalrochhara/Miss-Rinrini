...
from Rinrini import RinriniAPI, RinriniCli
from Rinrini.helper import custom_filter


@RinriniCli.on_message(custom_filter.command(commands=('getapi')))
...
@RinriniCli.on_callback_query(filters.create(lambda _, __, query: 'api_generate' in query.data))
async def generate_api_callback(client: RinriniCli, callback_query: CallbackQuery):
    user_id = callback_query.from_user.id 
    first_name = callback_query.from_user.first_name 
    username = callback_query.from_user.username

    operation, api_key = RinriniAPI.generate_api(user_id, first_name, username)
    if operation:
        text=(
            "Here is your **RinriniGban** API key:\n\n"
            f"`{api_key}`"
        )
    else:
