...
from Rinrini import GROUP_ANONYMOUS_BOT, RinriniCli
from Rinrini.database.chats_settings_mongo import get_anon_setting
from Rinrini.helper.chat_status import anon_admin_checker
...
def anonadmin_checker(func):
    ...
    Args:
        func ([type]): pass client: RinriniCli, message: Message
    """
    async def wrap(client, message: Message):
        ...
        @RinriniCli.on_callback_query(filters.create(lambda _, __, query: 'anonAdminConfirm_' in query.data))
        async def anon_callback(client: RinriniCli, callback_query: CallbackQuery):
